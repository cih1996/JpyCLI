...
func Execute() {
	...
	// 注意：file/version/update 子命令有自己的 --remote 处理逻辑，不走全局拦截
	if addr, args := extractRemoteFlag(os.Args[1:]); addr != "" {
		// file/version/update 子命令不走全局拦截，它们的 --remote 是目标服务器地址
		if len(args) > 0 && (args[0] == "file" || args[0] == "version" || args[0] == "update") {
			// 跳过拦截，让 Cobra 正常解析
